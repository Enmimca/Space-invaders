...
def dessiner():
    ...
    surface_nombre_projectiles = arial20.render("projectiles =" + str(nombre_projectiles), True, pygame.Color(0,255,255)) #on définit l'affichage du nombre de projectiles restants
    fenetre.blit(surface_nombre_projectiles, (10,30)) #emplacement de la surface d'écriture pour le nb de projectiles restants
    surface_vies_alien = arial20.render("Vies de l'alien = " + str(vies_alien), True, pygame.Color(0, 255, 255)) #on définit l'affichage du nombre de vies de l'alien
    ...
    for bombe in bombes:
        bombe = fenetre.blit(image_bombe, (position_alien[0] + 16,position_alien[1]))
...
#L'alien se décale de droite à gauche ou de gauche à droite
        if se_dirige_vers_la_droite:
            ...
            if random.randint(0, 15) == 1:
               bombes.append(bombe[0], bombe[1] + 5)
